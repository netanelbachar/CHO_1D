...
for i, value in enumerate(block_size_array):
    number_of_blocks, avg, stdv = block_averaging(cutoff, block_size=value, data=kin_est)
    avg_array[i] = avg
    stdv_array[i] = stdv
...
np.savez("block_avg_file", avg = avg_array, stdv = stdv_array, time=times, temp=temp_exp, pot=pot_est, kin=kin_est, heff=h_eff_change)
