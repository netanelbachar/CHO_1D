...
import math
import time
#Change initial position from QHO to
mass, w, hbar, kboltz, g, si = 1, 1, 1, 1, 1, 1
beta = 1
...
def expo1(x_long):
    ...
    x_diff = np.zeros(len(x_long))
    x = np.array(np.split(x_long, N_particles))
    for i in range(half):
        x_diff[i] = (x[0][i] - x[1][i])**2
    return x_diff
...
def rep_gaus_force1(g, si, x_long):
    ...
    x = np.array(np.split(x_long, N_particles))
    arg = expo1(x_long)
    for i in range(len(x_long)//2):           # 2 is number of particles
...
def langevin_dynamics(g_steps, dt, mass, beta, hbar, kboltz, w, wp, g, si, beads, N_particles):
    ...
    f_ext = rep_gaus_force1(g, si, x) / beads
